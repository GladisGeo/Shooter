...
def diffMenu(mainMenu):
    click = False
    go=True
    while go == True:
        screen.fill(MAROON)
        draw_text('DIFFICULTY: %s'%(mainMenu.difficulty), gameSubfont, GOLD, screen, SETTINGS.WIDTH/3, 20)
        ...
        if button_1.collidepoint((mx, my)):
            if click:
                ...
                mainMenu.difficultyMultiplier=-20 
                mainMenu.difficultyColor=SETTINGS.GREEN 
        if button_2.collidepoint((mx, my)):
            if click:
                pass
                mainMenu.difficulty='NORMAL' 
                mainMenu.difficultyMultiplier=0   
                mainMenu.difficultyColor=SETTINGS.YELLOW   
        if button_3.collidepoint((mx, my)):
            if click:
                pass
                mainMenu.difficulty='HARD'
                mainMenu.difficultyMultiplier=20
                mainMenu.difficultyColor=SETTINGS.RED  
        if button_4.collidepoint((mx, my)):
            if click:
                go=False
        pygame.draw.rect(screen, MAROON, button_1)
        draw_text('EASY', gameMenufont, GOLD, screen, 110, 260)
        pygame.draw.rect(screen, MAROON, button_2)
        draw_text('NORMAL', gameMenufont, GOLD, screen, 110, 330)
        pygame.draw.rect(screen, MAROON, button_3)
        draw_text('HARD', gameMenufont, GOLD, screen, 110, 400)
